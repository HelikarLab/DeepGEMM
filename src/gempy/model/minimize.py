...
# pool = parallel.pool(processes = CPU_COUNT)

def _single_syntehtic_lethality_step(rxn, m, objective_value, cutoff = 0.01):
    with m as m_copy:
        m_copy.reactions.get_by_id(rxn).knock_out()
        logger.info("Synthetic Lethality, knocked out reaction: %s" % rxn)
        d_objective_value = m_copy.slim_optimize()

        if d_objective_value < (cutoff * objective_value) or np.isnan(d_objective_value):
            return rxn
...
def single_synthetic_lethality(m, cutoff = 0.01, eliminate = [], tolerance = None, processes = None):
    ...
    with parallel.no_daemon_pool(processes = processes) as pool:
        ...
        rxn_ids = lfilter(bool, tq.tqdm(pool.imap(fn, pruned_rxns), total = len(pruned_rxns), desc = "Synthetic Lethality, checking for objective on knockout..."))
...
def get_fva_mapped_rxns(m, fn, *args, **kwargs):
    ...
    kwargs["loopless"]  = kwargs.get("loopless", True)
...
def minreact(m, growth_rate_cutoff = 1, tolerance = None, maintain = (BIGG_ID_ATPM,), jobs = CPU_COUNT):
    ...
    objective_reactions = list(linear_reaction_coefficients(minimized))
    objective_value = minimized.slim_optimize()
    ...
    with minimized as minimized_copy:
        ...
        pfba_essential_rxns = single_synthetic_lethality(minimized_copy,
            eliminate = maintain, processes = jobs)
        ...
        mle_rxns = [rxn.id for rxn in fva_mapped_rxns if rxn not in zero_flux_rxn_map]
